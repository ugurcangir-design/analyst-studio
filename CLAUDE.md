...
5. Prompt önceliği: ekrandaki **Özel Prompt** (`context_filter.json → ozel_prompt`, analiz-bazlı, varsayılanın YERİNE geçer) > `reference/prompts.json` (kalıcı override, **gitignore/makine-yerel** — tüm skill promptunu EZER, fork riskli) > `VARSAYILAN_PROMPTLAR` (base.py). **Domain kuralları (MBS eğitim):** `reference/domain-kurallari.md` (git'te İZLENİR, PII'siz) → `base._domain_kurallari_oku()` HER analiz promptuna (`_EK_KURAL_SKILL_IDS` + süreç/teknik özel-prompt yolları) `_ORTAK_EK_KURALLAR`'dan SONRA otomatik ekler. `<!-- KURALLAR-BASLANGIC -->` işaretinden sonrası boş/yorum iken enjekte edilmez (güvenli no-op). Terim sözlüğü + değişmez KURALLAR buraya; GERÇEK VERİ (tablo/örnek/Kafka/endpoint) RAG corpus'una (`reference/confluence`, `reference/services`). Yol haritası + kürasyon: `docs/AGENT-EGITIM.md`.
